...
from sources import Source, function, function2
...
if __name__ == "__main__":
    ...
    e1 = Source(50, 50, cp.asnumpy(s.t), dt, dx, dy, dx, 2*np.pi*50, lambda x : function2(x, 11000))
    ...
    dT = 1/1000
    t_ret = np.arange(0, 0.5, dT)

    fig, ax = plt.subplots(1, 1)
    im = ax.imshow(np.zeros((s.n, s.m)), cmap="RdBu", vmin=-2e-3, vmax=2e-3, aspect="equal", interpolation="bilinear")
    ...
    clb = fig.colorbar(im, orientation="horizontal", cax=cax, format='%.0e')
    ...
    n = 25
    A = np.memmap("output/Pressure_Field.npy", dtype='float32', mode='w+', shape=(s.n, s.m, 2*n))
    A[:, :, :] = np.zeros((s.n, s.m, 2*n))

    k = 0
    t0 = time.time()
    for current_time, P in s.solve(t_ret):
        if (tmin + tmax - n * dT) / 2 < current_time < (tmin + tmax + n * dT) / 2:
            A[:, :, k] = P
            k += 1
        ax.set_title("Time: {:4.0f} ms".format(current_time * 1000))
        im.set_data(P)
        plt.savefig(f"./output/2dfdtd_{int(current_time / dT):05}.png", dpi=180, bbox_inches='tight')
    print(f"Took : {time.time() - t0}")

    np.save("output/time.npy", t_ret[:n])